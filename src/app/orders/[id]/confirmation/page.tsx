...
export default function OrderConfirmationPage() {
  ...
  // Compute totals from product snapshots so we can show discount line clearly
  const itemsTotal = order.products.reduce((s, p) => s + p.price * p.quantity, 0)
  const hasDiscount = (order.discount ?? 0) > 0 || !!order.coupon_code
  const discountAmount = hasDiscount ? (order.discount ?? 0) : 0
  const taxableIncluded = Math.max(0, itemsTotal - discountAmount) // tax-included amount (product prices include GST)
  const gstAmount = Math.round(taxableIncluded - taxableIncluded / (1 + gstRate))
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 space-y-6 print:border-none print:shadow-none">
        ...
        <div className="border-t border-gray-300 pt-4 text-right">
          <div className="text-right text-sm pt-4 text-gray-700 space-y-1">
            <p>Items Total: ₹{itemsTotal.toLocaleString('en-IN')}</p>
            {hasDiscount && (
              <p>Discount ({order.coupon_code ?? 'promo'}): -₹{discountAmount.toLocaleString('en-IN')}</p>
            )}
                ...
                if (printWindow) {
                  printWindow.document.write(`
                    ...
                    <body>
                        ...
                        <p class="subtotal">Items Total: ₹${itemsTotal.toLocaleString('en-IN')}</p>
                        ${hasDiscount ? `<p class="subtotal">Discount (${order.coupon_code ?? 'promo'}): -₹${discountAmount.toLocaleString('en-IN')}</p>` : ''}
                        <p class="subtotal">Included GST (approx.): ₹${gstAmount.toLocaleString('en-IN')}</p>
                        <p class="subtotal">Delivery Charges: ₹${deliveryCharge.toLocaleString('en-IN')}</p>
